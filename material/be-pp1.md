...
├── models/
│   └── userModel.js        # User schema and database structure
├── controllers/
...
**❌ WRONG WAY - Storing passwords as plain text:**
```javascript
// NEVER DO THIS!
...
Hashed Value: "$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcg7b3XeKeUxWdeS86E36P4/DJm"
           ↓ (cannot reverse)
Original password is lost forever ✓
```
...
**Step 2:** Click **Body** tab → Select **raw** → Select **JSON**
...
|-----------|-------|----------|-----------------|
| Valid signup | `john@ex.com` | `"4wa95=Vx#!` | ✓ Success |
| Invalid email | `notanemail` | `"4wa95=Vx#!` | ✗ Error |
| Weak password | `john@ex.com` | `weak` | ✗ Error |
| Duplicate email | (existing) | `"4wa95=Vx#!` | ✗ Error |
| Correct login | `john@ex.com` | `"4wa95=Vx#!` | ✓ Success |
| Wrong password | `john@ex.com` | `WrongPass123!` | ✗ Error |
| Non-existent | `fake@ex.com` | `"4wa95=Vx#!` | ✗ Error |
| Empty fields | `` | `` | ✗ Error |
